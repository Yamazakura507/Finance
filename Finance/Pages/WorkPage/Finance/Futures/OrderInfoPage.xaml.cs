    ...
    { 
        ...
        loading.LoadingBackgorundWorker.RunWorkerAsync(new Thread(() =>
        {
            try
            {
                ...
                ViewScEn = DBModel.GetModel<View.ScalpingEntries>(((Models.ScalpingEntries)this.BindingContext).Id);

                if (ViewScEn is null) throw new Exception("У вас отсутствуют скальп хранилища");
                else
                {
                    MainThread.BeginInvokeOnMainThread(() => vScVSL.BindingContext = ViewScEn);
                }
            }
            catch (Exception ex)
            {
